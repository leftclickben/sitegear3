...
(function (_, sitegear3, storage) {
	...
	describe('Sitegear3 application lifecycle: persistence()', function () {
		...
		describe('When called with a valid driver', function () {
			beforeEach(function () {
				app.storageDrivers = { test: _.noop };
				spyOn(app.storageDrivers, 'test');
			...
			it('Instantiates the specified driver', function () {
				expect(app.storageDrivers.test).toHaveBeenCalledWith({ foo: 'foo', bar: 'baz' });
				expect(app.storageDrivers.test.callCount).toBe(1);
			});
		});
		...
		describe('When called with an unknown driver', function () {
			var error;
			beforeEach(function () {
				app.storageDrivers = { test: 'this should be a function, not a string' };
				try {
