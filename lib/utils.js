...
(function (_, path, glob) {
	...
	module.exports = {
		 ...
		 *
		 * globGetters(myObject, namePattern, globPattern, function () { ... });
		 * globGetters(namePattern, globPattern, function () { ... });
		 ...
		 * @param callback
		 */
		globGetters: function (destination, namePattern, globPattern, callback) {
			switch (arguments.length) {
				...
				case 3:
					if (_.isPlainObject(destination)) {
						...
						globPattern = namePattern;
						namePattern = /\/([a-zA-Z0-9_\-\.]+)\.js$/;
					} else {
						callback = globPattern;
						globPattern = namePattern;
						namePattern = destination;
						destination = {};
					}
					break;
			}
