...
(function (_) {
	...
	module.exports = function (app) {
		var categoryCollection = app.storage.collection('productCategory'),
			itemCollection = app.storage.collection('productItem');

		return {
			index: function () {
				return function (request, response, next) {
					categoryCollection.all(function (error, categories) {
						if (error) {
							next(error);
