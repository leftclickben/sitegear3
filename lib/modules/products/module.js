...
(function (_, schemaValidator) {
	...
	module.exports = function (app) {
		var categoryRepository = app.data.define('productCategory', schemaValidator(require('./schema/category.schema.json'))).on('error', function (error) {
				console.log('Product category repository encountered an error: ' + error); // TODO Something better
			}),
			itemRepository = app.data.define('productItem', require('./models/item'), schemaValidator(require('./schema/item.schema.json'))).on('error', function (error) {
				console.log('Product item repository encountered an error: ' + error); // TODO Something better
			});
