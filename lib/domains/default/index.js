	...
	module.exports = function (app) {
		var domain = {
			index: function (request, response, next) {
				...
				domain.pageRepository.get(path, function (error, page) {
					if (error) {
						next(error);
		...
		domain.pageRepository = app.data.define('page', schemaValidator(require('./schema/page.schema.json'))).on('error', function (error) {
