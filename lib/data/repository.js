...
(function (_, events) {
	...
	module.exports = function (connector, type, model, validators) {
		model = model || {};
		...
		var repository = _.extend(new events.EventEmitter(), {
				...
				set: function (key, value, callback) {
					...
					return this.validate(value, function () {
						connector.set(type, key, value, function (error) {
							callback(error);
							if (error) {
				...
				get: function (key, callback) {
					callback = callback || _.noop;
					connector.get(type, key, function (error, value) {
						if (value) {
				...
				keys: function (callback) {
					callback = callback || _.noop;
					connector.keys(type, function (error, keys) {
						callback(error, keys);
						if (error) {
							repository.emit('error', error);
						} else {
				...
				all: function (callback) {
					callback = callback || _.noop;
					connector.all(type, function (error, data) {
						callback(error, data);
						if (error) {
							repository.emit('error', error);
						} else {
				...
				remove: function (key, callback) {
					callback = callback || _.noop;
					connector.remove(type, key, function (error) {
						callback(error);
						if (error) {
							repository.emit('error', error);
						} else {
				...
				clear: function (callback) {
					callback = callback || _.noop;
					connector.clear(type, function (error) {
						callback(error);
						if (error) {
							repository.emit('error', error);
						} else {
