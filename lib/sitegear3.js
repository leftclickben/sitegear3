...
(function (_, path, http, https, connect, express, redis, storage, utils, defaults) {
	...
	module.exports = function () {
		...
		return _.extend(app, {
			...
			persistence: function (driverName, driverOptions) {
				...
				app.interfaces.storage.on('error', function (error) {
					console.log('Storage interface encountered an error from the ' + driverName + ' driver: ' + error);
				});
				return app;
			},
	...
	utils.loadPathAsGetters(path.join(__dirname, 'helpers', '*.js'), function (error, result) {
		module.exports.helpers = result;
	});

	// Expose module factories as getters.
	utils.loadPathAsGetters(path.join(__dirname, 'modules', '*.js'), function (error, result) {
//	utils.loadPathAsGetters(path.join(__dirname, 'modules', '**', '*.js'), /\/([a-zA-Z0-9_\-\.]+)\/module\.js$/, function () {
		module.exports.modules = result;
	});

	// Expose storage driver factories as getters.
	utils.loadPathAsGetters(path.join(__dirname, 'storage', 'drivers', '*.js'), /\/([a-zA-Z0-9_\-\.]+)\.js$/, function (error, result) {
	...
	module.exports.ready = function (timeout, callback) {
		...
		interval = setInterval(function () {
			console.log('interval', _.isObject(module.exports.helpers), _.isObject(module.exports.modules), _.isObject(module.exports.storageDrivers));
			if (Date.now() > started + timeout) {
				...
				callback(new Error('Timed out waiting for all resources to be ready after ' + timeout + 'ms.  Current status is: ' + readyStatus));
			}
