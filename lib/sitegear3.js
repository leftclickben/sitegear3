...
(function (_, path, http, https, connect, express, redis, storage, utils, defaults) {
	...
	utils.globGetters(path.join(__dirname, 'modules', '*.js'), function (error, result) {
//	utils.globGetters(path.join(__dirname, 'modules', '**', '*.js'), /\/([a-zA-Z0-9_\-\.]+)\/module\.js$/, function () {
		module.exports.modules = result;
	});

	// Expose storage driver factories as getters.
	utils.globGetters(path.join(__dirname, 'storage', 'drivers', '*.js'), /\/([a-zA-Z0-9_\-\.]+)\.js$/, function (error, result) {
