...
(function (_, path, http, https, connect, express, mergeDescriptors, storage, utils, defaults) {
	"use strict";
	...
	module.exports = function () {
		...
		return _.extend(app, {
			...
			stop: function () {
				...
				if (_.isObject(app.server) && _.isObject(app.server.http) && _.isFunction(app.server.http.close)) {
					app.server.http.close();
				}
				if (_.isObject(app.server) && _.isObject(app.server.https) && _.isFunction(app.server.https.close)) {
					app.server.https.close();
				}
			}
		});
	};
...
}(require('lodash'), require('path'), require('http'), require('https'), require('connect'), require('express'), require('merge-descriptors'), require('./storage/interface'), require('./utils'), require('./defaults.json')));
