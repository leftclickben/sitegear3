...
(function (_, path, http, https, connect, express, dataProvider, utils, defaults) {
		...
		return _.extend(app, {
			...
			module: function (name) {
				if (!app.modules.hasOwnProperty(name)) {
					var moduleFactory = require(path.join(__dirname, 'modules', name, 'module.js'));
	...
	utils.globGetters(/\/([a-zA-Z0-9_\-\.]+)\/connector\.js$/, path.join(__dirname, 'data', 'connectors', '**', 'connector.js'), function (error, result) {
	...
	module.exports.ready = function (timeout, callback) {
		...
		interval = setInterval(function () {
			...
			if (readyStatus.middleware && readyStatus.connectors) {
