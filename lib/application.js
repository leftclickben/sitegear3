...
(function (_, path, http, https, connect, express, dataProvider, utils, defaults) {
	...
	module.exports = function () {
		...
		return _.extend(app, {
			...
			persistence: function (driverName, driverOptions) {
				var driver = module.exports.drivers[driverName];
				if (_.isUndefined(driver)) {
					throw new Error('Unknown data driver specified: ' + driverName);
				}
				if (!_.isFunction(driver)) {
					throw new Error('Invalid data driver specified: ' + driverName);
				}
				app.data = dataProvider(driver(driverOptions));
				return app;
			},
	...
	// Expose data driver factories as getters.
	utils.globGetters(path.join(__dirname, 'data', 'drivers', '**', 'driver.js'), /\/([a-zA-Z0-9_\-\.]+)\/driver\.js$/, function (error, result) {
		module.exports.drivers = result;
	});
		...
		interval = setInterval(function () {
			if (Date.now() > started + timeout) {
				var readyStatus = {
					...
					drivers: _.isObject(module.exports.drivers)
				};
			...
			if (_.isObject(module.exports.middleware) && _.isObject(module.exports.modules) && _.isObject(module.exports.drivers)) {
				clearInterval(interval);
				callback();
			}
		}, 100);
