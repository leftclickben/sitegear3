...
(function (_, path, http, https, connect, express, mediator, utils, defaults) {
	"use strict";
		...
		return _.extend(app, {
			...
			component: function (name) {
				if (!app.components.hasOwnProperty(name)) {
					var factory = require(path.join(__dirname, 'components', name));
...
}(require('lodash'), require('path'), require('http'), require('https'), require('connect'), require('express'), require('./data/mediator'), require('./utils'), require('./defaults.json')));
