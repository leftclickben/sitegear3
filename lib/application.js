...
(function (_, path, http, https, connect, express, storage, utils, defaults) {
	"use strict";
	...
	module.exports = function () {
		...
		return _.extend(app, {
			...
			persistence: function (driverName, driverOptions) {
				...
				app.storage = storage(driver(driverOptions));
				return app;
			},
	...
	utils.globGetters(path.join(__dirname, 'storageDrivers', '**', 'driver.js'), /\/([a-zA-Z0-9_\-\.]+)\/driver\.js$/, function (error, result) {
	...
	module.exports.ready = function (timeout, callback) {
		...
		interval = setInterval(function () {
			...
			if (_.isObject(module.exports.middleware) && _.isObject(module.exports.modules) && _.isObject(module.exports.storageDrivers)) {
				clearInterval(interval);
				callback();
			}
		}, 100);
...
}(require('lodash'), require('path'), require('http'), require('https'), require('connect'), require('express'), require('./storage'), require('./utils'), require('./defaults.json')));
